<?php
	global $page_title;
	$page_title = "";
	include_once("./header.php");
?>


<div class="content-index">

	<div class="info">
		<div class="info-top-text">Сохраним наш мир <span class="info-green">чистым!</span></div>

		<h1>Компания «Алл Грин» основана в 2006 году как производственная фирма по производству моющих и чистящих средств бытового и промышленного назначения.</h1>

		<p><a href="/company/" title="О компании">Подробнее о компании</a>&nbsp;→</p>
	</div>

	<nav class="index-menu">
		<ul>
			<li>
				<a href="/products/" title="Продукция компании «Алл Грин»">
					<img src="/img/prods/1.png" alt="Продукция компании «Алл Грин»">
					<span>Продукция «Алл Грин»</span>
				</a>
			</li>
			<li>
				<a href="/awards/" title="Награды, дипломы, сертификаты">
					<img src="/img/prods/2.png" alt="Награды, дипломы, сертификаты">
					<span>Награды, дипломы</span>
				</a>
			</li>
			<li>
				<a href="/partners/" title="Партнеры компании">
					<img src="/img/prods/3.png" alt="Партнеры компании">
					<span>Наши партнеры</span>
				</a>
			</li>
			<li>
				<a href="/contacts/" title="Контактная информация">
					<img src="/img/prods/4.png" alt="Контактная информация">
					<span>Контактная информация</span>
				</a>
			</li>
		</ul>
	</nav>

	<div class="index-sections">

		<div class="index-sect-left">
			<img src="/img/skidka.jpg" alt="Скидка 10% на все в честь юбилея компании">
		</div>

		<div class="index-sect-right">
			<h2>ПРОИЗВОДСТВО ПОД СВОЕЙ МАРКОЙ</h2>
			<p>Одно из направлений компании «Алл Грин», является производство бытовой химии под торговой маркой наших партнеров (СТМ - своя торговая марка).</p>
			<p></p>
			<p>Продукция «Алл Грин» отвечает высочайшему европейскому качеству, безопасна для окружающей среды и человека, экономична в использовании.</p>
		</div>

	</div>

</div>


<?php
	include_once("./footer.php");
?>
